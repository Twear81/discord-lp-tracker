...
export const getLangServer = async (serverId: string): Promise<string> => {
    try {
        const existingServer: Model<any, any> | null = await Server.findOne({ where: { serverid: serverId } });
...
export const addPlayer = async (serverId: string, accountName: string, tag: string, region: string): Promise<void> => {
    try {
        ...
        const existingServer: Model<any, any> | null = await Server.findOne({ where: { serverid: serverId } });
        const existingPlayer: Model<any, any> | null = await Player.findOne({ where: { serverid: serverId, accountnametag: accountNameTag, region } });
